...
public class Consumer2 {
    ...
    @KafkaListener(id = "id2", topics="topic01", groupId = "myGroup")
    public void consumeFromTopic(@Payload List<String> msgs, @Header(KafkaHeaders.RECEIVED_PARTITION_ID) List<Integer> partitions, @Header(KafkaHeaders.OFFSET) List<Long> offsets) {
        ...
        for(int i=0; i < msgs.size(); i++){
            System.out.println("consumed message :: "+ msgs.get(i) + " from consumer 2 in partition :: "+partitions.get(i) + " Now current offset is :: " + offsets.get(i));
